...
public class DynamicPlanning {
     ...
     *
     * // 动态规划：
     * // 定义f(n)：f(n)为前n天的最大利润。（即定义dp数组）
     * // 状态转移方程：f(i) = max(f(i-1), prices[i] - min(prices[0:i]))；
     * //             前i天的最大利润等于前i-1天的最大利润和第i天卖出的最大利润的最大值（注意前i天的最大利润和第i天卖出的最大利润的区别）
     * 
     ...
     *
     * 一、定义状态
     *  设dp[i,0] 为第i天，不持有股票的最大收益，
     *  dp[i,1] 为第i天，持有当天股票的最大收益
     * 二、初始值
     *  dp[0,0] = 0;
     *  dp[0,1] = -prices[0];
     * 三、转移方程
     *  dp[i,0] = max(dp[i-1,0], dp[i-1,1] + prices[i]); 为什么能直接+prices[i]，因为初始值时 = -prices[0] ，所以等同于prices[i] - prices[i-1]
